...
- Realistic materials (on by default) add self-hosted color, normal and roughness maps for wood, stone flooring and clay roofs, plus plaster and upholstery surface detail. Metal and ceramic use environment reflections. Turn the option off for simple surfaces; failed texture sets fall back automatically. The 13 JPEG maps total 5.57 MiB and use 1K resolution with shared GPU textures. Sources and hashes are recorded in `viewer/public/textures/sources.json`; see [material notes](docs/viewer-materials.md).
