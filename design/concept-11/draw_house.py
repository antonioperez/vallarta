...
OUT = ROOT / 'las-juntas-concept-11.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | Longer car and deeper court | Concept 11')
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 11  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
    ...
    txt(W-48, 32, f'{sheet:02d} / 09', 8, MUTED, align='right')
...
def page_plans():
    header(1,'A longer car, with a deeper front court.',
...
def page_appearance():
    ...
    y=top-image_size-12
    y=para(48,y,W-96,'House and terrace move back 0.30 m. The 4.80 m car retains a 0.55 m boundary gap and 0.45 m nose gap; closed-gate clearance is approximately 0.25 m before hardware. Rear: 3.70 m total depth, including the 3.00 m terrace. Views are illustrative; dimensioned plans govern.',8.5,MUTED,leading=11.5)-15
    colw=(W-120)/2
    ...
    para(48,77,W-96,'Stair windows: separate inward-opening units with solid wall/floor between. Sill/head heights, sash clearance, safe cleaning reach, privacy, shading and glazing performance need detailing. Gate operation: page 9. Gate slats do not establish acoustic performance.',7.4,MUTED,leading=9)
    c.showPage()
...
def verify_geometry():
    ...
    baseline = json.loads((ROOT.parent/'concept-10'/'area-schedule.json').read_text())
    changed_keys = {'areas','areas_square_feet_reference','parking_revision','sliding_gate','checks'}
    for key,value in baseline.items():
        if key not in changed_keys:
            assert json.loads(json.dumps(payload[key])) == value, f'Unexpected interior/program change: {key}'
    for key,value in baseline['areas'].items():
        if key not in {'front_zone','rear_zone'}:
            assert isclose(areas[key],value), f'Unexpected floor area change: {key}'
    for key,value in baseline['sliding_gate'].items():
        if key != 'exit_corridor':
            assert json.loads(json.dumps(GATE[key])) == value, f'Unexpected physical gate change: {key}'
    payload['checks'] += ['interior, heights, kitchen, stair, acoustic and service records equal Concept 10',
                          'physical gate geometry equals Concept 10; only checked exit corridor extends']
    (ROOT/'area-schedule.json').write_text(json.dumps(payload,indent=2)+'\n')
    render_geometry = json.loads((ROOT.parent/'concept-10'/'rendering-geometry.json').read_text())
    render_geometry['adopted'].update(front_court_depth=SITE['front_depth'],rear_zone_depth=SITE['rear_depth'])
    render_geometry['site_lot_coordinates_m'] = SITE
    render_geometry['parking_m'] = PARKING
    render_geometry['gate_lot_coordinates_m'] = GATE
    render_geometry['visual_assumptions_only']['rear_boundary_wall'] = 'low wall top in rear foreground is illustrative; enclosure design not selected'
    (ROOT/'rendering-geometry.json').write_text(json.dumps(render_geometry,indent=2)+'\n')

if __name__=='__main__':
    verify_geometry()
    page_plans(); page_service_detail(); page_heights(); page_acoustic_details(); page_construction(); page_schedule(); page_kitchen(); page_appearance(); page_gate(); c.save()
    print(json.dumps({'pdf':str(OUT),'pages':9,'combined_floor_area_m2':round(areas['combined_floor'],1),'terrace_m2':18}))
