...
OUT = ROOT / 'las-juntas-concept-07.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | Updated architectural character | Concept 07')
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 07  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
    ...
    txt(W-48, 32, f'{sheet:02d} / 07', 8, MUTED, align='right')
...
def building(p, upper=False):
    ...
    if not upper:
        ...
        labels=[(2,3.55,'G1  GUEST'),
                (6.75,5.92,'G4  KITCHEN'),(2.20,10.10,'G5  LIVING'),
...
def page_plans():
    ...
    for title,body in [
        ('Ground / social + guest','G1 queen room; G2 front shared bath; G3 laundry; G4 kitchen; G5 living/dining; G6 entry; G7 stair; G8 enclosed service passage.'),
...
def page_appearance():
    header(7,'The same warm architectural character',
    ...
    y=top-image_size-12
    y=para(48,y,W-96,'Concept views informed by the current plans and measured elevation guides. The dimensioned plans govern; perspective, furnishings and planting are illustrative. Roof/parapet levels, window sill/head heights and site levels remain to be detailed.',8.5,MUTED,leading=11.5)-15
    colw=(W-120)/2
    for xx,title,rows in [
        (48,'ADOPTED HEIGHTS / FRONT COURT',[
            dimension_row('Ground clear ceiling',HEIGHTS['ground_clear']),
            dimension_row('Upper clear ceiling',HEIGHTS['upper_clear']),
            dimension_row('Floor to floor',HEIGHTS['floor_to_floor']),
            dimension_row('Front court depth',5.5)]),
...
if __name__=='__main__':
    verify_geometry()
    page_plans(); page_service_detail(); page_heights(); page_acoustic_details(); page_construction(); page_schedule(); page_appearance(); c.save()
    print(json.dumps({'pdf':str(OUT),'pages':7,'combined_floor_area_m2':round(areas['combined_floor'],1),'terrace_m2':18}))
