...
OUT = ROOT / 'las-juntas-concept-13.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | A windows with B clay roofs | Concept 13')
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 13  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
...
def page_plans():
    header(1,'A windows, with B clay roofs.',
    ...
    for title,body in [
        ('Ground / social + guest','G1 queen room; G2 front shared bath; G3 laundry; G4 revised kitchen (page 7); G5 living/dining; G6 entry; G7 stair; G8 service passage. Sliding vehicle gate: page 9.'),
    ...
    para(x,y,width,'Setbacks, boundary-wall permission, floor elevation, structure and services need local review. Selected roof and window geometry: page 10. No surveyed north is asserted.',9)
...
def page_appearance():
    ...
    y=top-image_size-12
    y=para(48,y,W-96,'A supplies the high bathroom ribbon, two wider stair windows, ivory plaster and slim entry shade. B supplies the clay hip roof and tiled terrace canopy. The latest 5.80 m court, 4.80 m car and 0.70 m rear strip remain. Views are illustrative; measured geometry on page 10 governs.',8.5,MUTED,leading=11.5)-15
    colw=(W-120)/2
...
def verify_geometry():
    ...
    payload['checks'] += ['approved kitchen coordinates retained','fridge and counter do not overlap','appliances contained in counter and reserved bay','0.65 m cooktop-to-sink preparation surface','fridge 90 degree sweep clears fixed cabinets and chairs with 0.30 m pull-out','chair pull-out route reduction and shared sink work area documented','front court reconciles 0.55 + 4.80 + 0.45 = 5.80 m']
    ...
    # Preserve the current site, interior and operating clearances from Concept 11.
    baseline = json.loads((ROOT.parent/'concept-11'/'area-schedule.json').read_text())
    changed_keys = {'vertical_design_notes','checks'}
    ...
    payload['checks'] += ['All retained Concept 11 site, interior, area, kitchen, height, stair, acoustic and gate records match'] + style_checks
    ...
    render_geometry = json.loads((ROOT.parent/'concept-11'/'rendering-geometry.json').read_text())
