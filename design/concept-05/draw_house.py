...
OUT = ROOT / 'las-juntas-concept-05.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | Adopted ceiling heights and revised stair | Concept 05')
...
rooms = [
    ...
    ('U5', 'Primary bedroom', 'Upper', 4.95, 4.95, 3.85, 4.35),
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 05  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
    ...
    txt(W-48, 32, f'{sheet:02d} / 05', 8, MUTED, align='right')
...
def building(p, upper=False):
    ...
    if not upper:
        ...
        p.rect(5.2,4.95,.8,.72,PAPER,INK,.5)
        p.text(5.6,5.24,'FR',6.5)
        p.rect(6,4.95,2.8,.65,WOOD,INK,.5)
        p.rect(8.15,5.6,.65,1.05,WOOD,INK,.5)
        p.rect(6.35,5.06,.75,.43,PAPER,INK,.4)
        for xx in [6.53,6.89]:
            for yy in [5.17,5.38]: p.circle(xx,yy,.06,PAPER,INK)
        p.rect(8.25,5.9,.43,.65,PAPER,INK,.4)
        p.table(5.5,7.55)
...
def page_plans():
    header(1,'Adopted heights, with a revised stair.',
    ...
    y=section(x,y,'Heights and the longer stair',width)
    y=para(x,y,width,'Ground ceiling: 3.00 m / 9.84 ft. Upper ceiling: 2.80 m / 9.19 ft. The 3.40 m / 11.15 ft rise uses 20 risers. The stair bay extends 0.30 m into the kitchen/primary-room side; see page 3 for the height and stair study.',9.5)-18
...
def page_service_detail():
    ...
    y=para(48,y,518,'G6 leads through G8 to the bathroom and recessed laundry. The bathroom door opens inward. Stacked machines are shown in an open alcove; loading temporarily uses the passage. Verify appliance depth, services and any enclosure before fixing cabinetry.',9.5,MUTED)
...
def page_heights():
    ...
    s.text(2.4,3.16,'FLOOR / SERVICES ALLOWANCE',7.5,INK,True)
    ...
    y=para(48,y,518,'The 0.40 m allowance includes structure, finishes and services; it is not a slab specification. Ground-floor elevation above grade, roof build-up and parapet height remain to be set.',9,MUTED)
    ...
    txt(x,y,'The plan changes by 0.30 m',11,TERRACOTTA,True)
    y=para(x,y-12,width,'The longer bay moves the kitchen counter and primary-room front wall toward the rear. The king bed remains in place; the primary room is now 3.85 x 4.35 m.',10)-18
...
def page_schedule():
    header(4,'Area, circulation and budget checks',
    ...
    y=para(left,y,width,'Each shared bathroom has one toilet. Upstairs, the toilet is in the main room with no separate compartment. Laundry opens to G8, a 1.45 m² / 15.6 ft² passage. Use privacy glazing at wet-room windows. Eight guests assumes sharing each double bed.',9)
    ...
    y=para(right,y,rw,'Provisional allowances, excluding land; no local quotation. Reprice the taller walls, revised stair, structure and bathroom scope before accepting this target. Preserve drainage, waterproofing and the patio when adjusting finishes or joinery.',9)-17
...
def page_appearance():
    header(5,'The same warm architectural character',
           'Style reference retained from concept 01 / facade proportions and openings to follow the adopted heights and revised plans')
    ...
    y=top-dh-18
    y=para(48,y,W-96,'Earlier AI-generated style reference, retained for materials and general character only. It has not been regenerated for the adopted heights or interior revisions. Use the plans and height study for dimensions; revise facade proportions and privacy glazing during design development.',8.5,MUTED)-20
    colw=(W-144)/3
...
def verify_geometry():
    ...
    for x,y,w,h in [(5.2,4.95,.8,.72),(6,4.95,2.8,.65),(8.15,5.6,.65,1.05)]:
...
if __name__=='__main__':
    verify_geometry()
    page_plans(); page_service_detail(); page_heights(); page_schedule(); page_appearance(); c.save()
    print(json.dumps({'pdf':str(OUT),'pages':5,'combined_floor_area_m2':round(areas['combined_floor'],1),'terrace_m2':18}))
