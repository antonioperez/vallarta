...
OUT = ROOT / 'las-juntas-concept-10.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | Exposed stair windows | Concept 10')
...
GATE = {'opening_x':(.80,4.40),'closed_leaf':(.70,.22,3.80,.08),
        ...
        'exit_corridor':(1.35,-.60,2.75,5.90),
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 10  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
...
def page_appearance():
    header(8,'The same warm architectural character',
           'Two exposed stair windows / current opening layout, adopted floor heights and compact outdoor spaces')
    ...
    y=top-image_size-12
    y=para(48,y,W-96,'Front: the terracotta screen is removed; two separate stair windows sit in cream stucco, one per floor. The complete vehicle gate remains fully open, sliding right behind the fixed fence. Dimensioned plans govern; views are illustrative. Parking gaps are nominal before gate hardware.',8.5,MUTED,leading=11.5)-15
    colw=(W-120)/2
    for xx,title,rows in [
        (48,'FRONT COURT / PARKING CLEARANCES',[
            ...
            ('Rear / front nominal bumper gaps','0.55 / 0.45 m','1.80 / 1.48 ft')]),
...
def page_gate():
    ...
    for opened,x,title in [(False,72,'CLOSED / ONE COMPLETE VEHICLE GATE'),
        ...
        p=Plan(x,380,scale=47)
        p.rect(0,0,10,5.5,FLOOR,INK,.7)
        if opened: p.rect(*GATE['exit_corridor'],HexColor('#DEE8D4'),None)
        p.rect(1,5.5,9,.2,INK,None)
        p.text(6.25,5.91,'HOUSE FRONT / SAME 5.50 m COURT',8,INK,True)
        p.rect(*PARKING['car'],HexColor('#C6CDCA'),INK,.55)
        p.rect(1.91,3.38,1.63,.75,WATER,INK,.3)
        p.rect(1.91,1.15,1.63,.58,WATER,INK,.3)
        p.text(2.725,2.9,'4.50 m CAR',8,INK,True)
        p.text(2.725,2.57,'NOSE TO HOUSE',6.6,MUTED)
        p.line(9.1,.45,9.1,4,WOOD,.9*p.s)
        p.line(9.1,4,5.8,4,WOOD,.9*p.s)
        p.line(5.8,4,5.8,5.5,WOOD,.9*p.s)
        draw_gate(p,opened)
    ...
    y=para(right,y,rw,'Reserve the complete sliding space for the gate and its hardware. Keep planting, storage and the pedestrian route outside it. The courtyard depth and car position are retained; the vehicle opening is widened from 3.20 to 3.60 m.',9.2)-14
...
def verify_geometry():
    ...
    payload['checks'] += ['closed gate covers full vehicle opening','open gate fits reserved runback and clears vehicle route','gate travel clears fixed posts and fence','pedestrian entrance stays outside sliding reserve']
