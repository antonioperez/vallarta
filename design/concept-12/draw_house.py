...
OUT = ROOT / 'las-juntas-concept-12.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | A windows with B clay roofs | Concept 12')
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 12  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
    ...
    txt(W-48, 32, f'{sheet:02d} / 10', 8, MUTED, align='right')
...
if __name__=='__main__':
    verify_geometry()
    page_plans(); page_service_detail(); page_heights(); page_acoustic_details(); page_construction(); page_schedule(); page_kitchen(); page_appearance(); page_gate(); page_selected_exterior(); c.save()
    print(json.dumps({'pdf':str(OUT),'pages':10,'combined_floor_area_m2':round(areas['combined_floor'],1),'terrace_m2':18}))
