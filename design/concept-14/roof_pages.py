...
C=None  # Assigned the main document canvas by draw_house.py
...
def header(n,title,sub):
 rect(0,0,W,H,PAPER,None);text(M,H-39,'LAS JUNTAS',12,ACCENT,True);text(W-M,H-39,'CONCEPT 14 / ROOF COORDINATION / 12 SEPTEMBER 2026',8.5,MUTED,align='right')
 text(M,H-78,title,26,INK,True);text(M,H-100,sub,10,MUTED)
 line(M,49,W-M,49);text(M,32,'PROPOSED ROOF COORDINATION - NOT FOR CONSTRUCTION',8,ACCENT,True);text(W-M,32,f'{n+10:02d} / 13',8,MUTED,align='right')
...
def page_plan():
 ...
 y=para(x,y,w,'Concept 12 and the original style study remain intact. These pages propose drainage relationships; they do not resize the roof, alter room areas or establish a final drainage installation.',9,MUTED)
 assert y>68,y
 C.showPage()
...
def page_handoff():
 header(3,'Give the designer quantities and decisions.', 'Retained Concept 12 roof quantities before any gutter inset / no product order, pipe sizing or construction budget established')
