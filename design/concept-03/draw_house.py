...
OUT = ROOT / 'las-juntas-concept-03.pdf'
W, H = landscape(A3)
INK = HexColor('#293A36')
...
c.setTitle('Las Juntas | Indoor laundry and bathroom passage | Concept 03')
...
rooms = [
    ...
    ('G8', 'Indoor service passage', 'Ground', 2.35, 1.5, 1.45, 1),
...
def header(sheet, title, subtitle):
    ...
    txt(W-48, H-40, 'CONCEPT 03  /  3 BED + 2 BATH  /  12 SEPTEMBER 2026', 9, MUTED, align='right')
    ...
    txt(W-48, 32, f'{sheet:02d} / 04', 8, MUTED, align='right')
...
class Plan:
    ...
    def front_service(self, detail=False):
        # The east bay contains recessed appliances and a separate, open passage.
        ...
        # Opening through the existing hall wall; no laundry door in the route.
        self.rect(3.79,1.5,.17,1,FLOOR,None)
...
def page_plans():
    header(1,'Indoor access to laundry and bathroom.',
    ...
    y=section(x,y,'Your swap, with an indoor passage',width)
    y=para(x,y,width,'G2 moves to the left; G3 sits beside the entry hall. G8 provides a 1.00 m / 3.28 ft-wide route behind the recessed laundry bay to the bathroom. The bathroom remains buffered from the kitchen. See the enlarged plan on page 2.',9.5)-18
    y=section(x,y,'Room arrangement',width)
    for title,body in [
        ('Ground / social + guest','G1 queen room; G2 front shared bath; G3 stacked laundry bay; G4 kitchen; G5 living/dining; G6 entry; G7 stair; G8 indoor passage.'),
...
def page_service_detail():
    header(2,'A short indoor passage solves the access problem.',
           'Enlarged ground-floor service area / 1:25 on A3 at 100% / meter and feet reference columns below')
    p=Plan(95,340,scale=S*4)
    ...
    p.text(1.9,-.71,'FRONT COURT / STREET SIDE',8,MUTED)
    p.dimh(.2,2.2,-.29,'2.00 m')
    p.dimh(2.35,3.8,-.29,'1.45 m')
    p.dimv(-.30,.2,2.5,'2.30 m')
    # Dimension within the passage, apart from the circulation arrow.
    p.line(3.61,1.5,3.61,2.5,MUTED,.5)
    for yy in (1.5,2.5): p.line(3.55,yy,3.67,yy,MUTED,.6)
    p.text(3.29,1.71,'1.00 m',8,MUTED)
    x=700; width=W-x-48; y=H-139
    y=section(x,y,'Entry hall > passage > bathroom',width)
    y=para(x,y,width,'Turn left from G6 into the short G8 passage. The bathroom door is at the end. The stacked washer/dryer faces this same passage from the recessed G3 bay, so both are reached indoors.',11)-22
    y=section(x,y,'A swap with one partition adjustment',width)
    y=para(x,y,width,'Move the dividing wall so the bathroom keeps its 2.00 x 2.30 m footprint. The remaining 1.45 m-wide bay holds the stacked laundry at the front and the 1.00 m-wide passage behind it.',10)-22
    y=section(x,y,'What this layout assumes',width)
    y=para(x,y,width,'Stacked appliances and an open laundry alcove are shown. Appliance depth, hose space, drainage, dryer ventilation and any future enclosure need to follow the selected equipment. Loading laundry temporarily uses part of the passage.',10)-22
    y=section(x,y,'The door stays inside the bathroom',width)
    y=para(x,y,width,'The nominal 0.85 m / 2.79 ft door swings into G2, away from the passage. The exterior laundry entrance is removed. The bathroom still shares no wall with the kitchen; the bedroom and circulation remain between them.',10)-22
    y=para(x,y,width,'Dimensions are schematic planning targets, not a clearance or accessibility certification. Final wall finishes, fixtures and door hardware need detailed review.',9,MUTED)
    assert y>65, 'Service-detail notes reach footer'
    y=section(48,233,'Service-area dimensions',570)
    rows=[]
    ...
    y=unit_table(48,y,570,rows,rowh=25,size=10)-12
    para(48,y,570,'Dashed green arrows show indoor access. The laundry bay includes appliance/service depth; the passage is a separate circulation zone.',9,MUTED)
    c.showPage()

def page_schedule():
    header(3,'Area, circulation and budget checks',
    ...
    schedule=[]
    labels=[('G1','Ground queen room'),('G2','Ground shared bath / front'),('G3','Recessed laundry bay'),('G5','Living + dining'),('G4','Kitchen / transition'),('U1','Upper two-double room'),('U2','Upper shared bath, incl. WC'),('U5','Primary bedroom'),('U4','Primary dressing / storage')]
    ...
    y=para(left,y,width,'Both full bathrooms have indoor shared access. Laundry opens to the new G8 passage; its separate area is 1.45 m² / 15.6 ft². Use privacy glazing at front wet-room windows. Eight guests assumes two people share each double bed.',9)
...
def page_appearance():
    header(4,'The same warm architectural character',
    ...
    y=top-dh-18
    y=para(48,y,W-96,'Earlier AI-generated style reference, retained for materials and massing only. It has not been regenerated for the bathroom/laundry swap. Front wet-room openings need privacy glazing; use the revised floor plans for the room arrangement.',8.5,MUTED)-20
    colw=(W-144)/3
...
def verify_geometry():
    ...
    payload={'units':'meters; areas in square meters','areas':areas,
             ...
             'service_access':{'route':['G6 entry hall','G8 indoor passage','G2 bathroom'],'laundry_access':'G3 opens directly to G8','passage_width_m':passage[6],'passage_width_ft':feet(passage[6]),'bathroom_door_opening_m':.85,'laundry_type':'stacked washer/dryer in an open recessed bay','exterior_laundry_door':False},
             'checks':['three bedrooms and two full bathrooms, one bathroom per floor','downstairs bathroom has no shared kitchen wall; 2.0 m plan-depth separation','lot allocation reconciles','room rectangles inside floor envelopes','room rectangles do not overlap','service-area rectangles and partition reconcile','passage connects hall, bath opening and laundry bay','stacked appliance footprint stays outside passage','stair rise/run arithmetic','budget allocation reconciles'],
...
if __name__=='__main__':
    verify_geometry()
    page_plans(); page_service_detail(); page_schedule(); page_appearance(); c.save()
    print(json.dumps({'pdf':str(OUT),'pages':4,'combined_floor_area_m2':round(areas['combined_floor'],1),'terrace_m2':18}))
